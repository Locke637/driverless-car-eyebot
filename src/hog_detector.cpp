...
int main(int argc, char** argv) {
  try {
    ...
    const double c_val = get_option(parser, "c", 8.0);
    ...
    const double filter_val = get_option(parser, "filter", 0.2);
    const double norm = get_option(parser, "norm", 0.01);
    ...
    scanner.set_detection_window_size(56,56);
    ...
    remove_unobtainable_rectangles(trainer, images_train, sign_boxes_train);
